...
#+BEGIN_SRC emacs-lisp :results output silent
  (use-package evil-cleverparens
    :straight (:host github :repo "luxbock/evil-cleverparens" :branch "master")
    :general
    (evil-inner-text-objects-map
    ...
    (when (boundp 'config-local-dropbox-folder)
      (add-to-list 'org-capture-templates
                   `("i" "Standard Inbox" entry
                     (file ,(concat config-local-dropbox-folder "/notes/inbox.org"))
                     "* TODO %? \nLOGGED: %U"))))
...
#+BEGIN_SRC emacs-lisp :results output silent
  (use-package darkroom
    ...
    :init
    (evil-ex-define-cmd "write[r]" 'darkroom-tentative-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results output silent
  (use-package ob-hy
    :straight (:host github :repo "brantou/ob-hy" :branch "master")
    :config
    (require 'ob-hy))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results output silent
  (use-package powershell.el
    :straight (:host github :repo "jschaf/powershell.el" :branch "master")
...
* COMMENT Org Babel Eshell

[[https://github.com/howardabrams/dot-files/blob/master/emacs-eshell.org][Potentially interesting things]]

Time to /really/ start going down the rabbit hole

cribbed from [[https://old.reddit.com/r/emacs/comments/aiyo27/org_babel_inserting_text_for_me_inside_source/eerphz9/][reddit]]

This is currently throwing an error and causing no end of frustration
for me.

But the literate devops dream must go on.

#+BEGIN_SRC emacs-lisp :results output silent :eval never
  (defun org-babel-execute:eshell (body _params)
    (with-temp-buffer (ignore-errors (eshell-command body t)) (buffer-string)))
#+END_SRC

** COMMENT Debugging the thing
#+BEGIN_SRC emacs-lisp :results output silent :var end=1 :eval never
  (get-text-property (1- end) 'comment)
#+END_SRC

raises an exception, which raises hell
