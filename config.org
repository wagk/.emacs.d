...
#+begin_src emacs-lisp :results output silent
  (with-eval-after-load 'ivy
    ...
    (evil-ex-define-cmd "Tmm" #'(lambda ()
                                  ;; TODO: have counsel-bookmark run
                                  ;; earlier so if we cancel we don't
                                  ;; get a new tab
                                  (interactive)
                                  (require 'counsel)
                                  ...
                                  ;; `tab-bar-new-tab-choice' assumes
                                  ;; that a buffer will be returned,
                                  ;; however, `bookmark' does not
                                  ;; necessarily store buffers, so a
                                  ;; translation between filenames/paths
                                  ;; to buffers must be done.
                                  (if (>= emacs-major-version 27)
                                      (tab-bar-new-tab)
                                    (require 'eyebrowse)
                                    (my-new-evil-tab nil))
  ...
  (with-eval-after-load 'consult
    ...
    (evil-ex-define-cmd "Tmm" #'(lambda ()
                                  ...
                                  (if (>= emacs-major-version 27)
                                    ...
                                    (require 'eyebrowse)
                                    (my-new-evil-tab nil))
                                  (funcall-interactively 'consult-bookmark))))
#+end_src
    ...
    (defun --select-org-agenda-file ()
      ...
      (let ((targets
             (->> org-agenda-files
                  ...
                  (-flatten))))
        (completing-read "Capture target: "
                         targets
                         nil t nil nil "todo")))
...
#+begin_src emacs-lisp :results output silent
  (use-package org-agenda
    ...
    ;; TODO: use `define-advice' instead
    (with-eval-after-load 'ivy
    ...
    (defun --with-selected-org-roam-directory (org-roam-func)
      ...
      (let ((repos (or --org-roam-repository-list `(,org-roam-directory))))
        (if (eq 1 (safe-length repos))
          ...
          (let ((repo (completing-read "repo: " repos nil t)))
...
#+begin_src emacs-lisp :results output silent
  (use-package magit
    ...
    (with-eval-after-load 'selectrum
      (customize-set-value 'magit-completing-read-function #'selectrum-completing-read))
...
#+begin_src emacs-lisp :results output silent
  (use-package scratch
    ...
    (evil-define-command --scratch (&optional bang)
      "Opens org-mode by default, asks for mode when ! is supplied"
      (interactive "<!>")
      (if bang
    ...
    (with-eval-after-load 'consult
      ...
      (defun --compile-command-completing-read ()
        ...
        (let ((command (completing-read "Compile command: "
                                        ...
                                        nil
                                        compile-command))
              (default-directory (or (cdr (project-current))
                                     default-directory)))
          (compile command)))
    ...
    (defun --project-find-doc ()
      ...
      (let* ((case-fold-search t) ; ignore cases
             ...
             (readme (completing-read "README: " readme-list
                                     nil t nil nil "README")))
        (find-file (f-join project-dir readme))))

    (my-evil-define-split-vsplit-cmd "readme" #'--project-find-doc)
...
* Selectrum/Consult/Embark

** Selectrum
Note that =ivy-mode= needs to be disabled and =selectrum-mode= needs to be
enabled before you can see changes.
...
** Consult
- =consult-buffer= and =consult-bookmark= has this interesting property of
  showing what is going on live (sort of like telescope in nvim?)
...
*** Marginalia
Known issue with annotators making some consults look horrifying ([[https://github.com/minad/marginalia/issues/95][link]]).
