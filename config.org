...
#+begin_src emacs-lisp :results output silent
  ...
  ;; (advice-remove 'transient--post-command #'--transient--post-command-hack)
#+end_src
** Magit
...
#+begin_src emacs-lisp :results output silent
  (use-package prescient
    :straight (:host github :branch "main" :repo "radian-software/prescient.el")
    :custom
...
* To Do and Done files
This is still fairly experimental

#+begin_src emacs-lisp :results output silent
  ;; NOTE (pangt): The ordering of these vars are such that they will
  ;; end up `nil' in a normal init sequence.
  ...
  (evil-ex-define-cmd "done[file]" #'(lambda () (find-file --done-file)))
  (evil-ex-define-cmd "todo[file]" #'(lambda () (find-file --todo-file)))
#+end_src
