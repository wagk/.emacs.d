...
(use-package obsidian
  ...
  :config
  (define-advice obsidian-file-p (:filter-args (&optional file-list) ignore-lock-files)
    "Ignore all files that start with `.#' and dotfile directories."
    ;; (message "file: %s" file-list)
    (cl-flet* ((is-not-lockfile-p (&optional file)
                (and file
                     (->> file
                          (f-filename)
                          (s-prefix-p ".#")
                          (not))))
               (is-not-git-file-p (&optional file)
                (and file
                     (->> file
                          (s-contains-p "/.git")
                          (not)))))
      (-filter (-andfn #'is-not-lockfile-p #'is-not-git-file-p) file-list)))
...
(use-package chatgpt-shell
  ...
  :config
  (defconst --chatgpt-shell-system-prompt
    (s-join "\n"
     '("Always show code snippets in markdown blocks with language labels."
       "Always write the summary first."
       "Always explain step-by-step and in detail."
       "Never simplify the answer."
       "For code, always write test cases")))
  (add-to-list 'chatgpt-shell-system-prompts --chatgpt-shell-system-prompt)
  (customize-set-variable 'chatgpt-shell-system-prompt --chatgpt-shell-system-prompt))
#+end_src
