...
#+begin_src emacs-lisp :results output silent
  (use-package org-agenda
    ...
    ;;                                        deft-directory)))
    (when (and (boundp 'config-local-dropbox-folder)
               (boundp 'config-local-syncthing-folder))
      (let ((dropbox-todo (concat config-local-dropbox-folder "/todo.org"))
            (sync-todo (concat config-local-syncthing-folder "/todo.org")))
        (when (file-exists-p sync-todo)
          (customize-set-variable 'org-agenda-files
                                  (add-to-list 'org-agenda-files
                                               sync-todo)))
        (when (file-exists-p dropbox-todo)
          (customize-set-variable 'org-agenda-files
                                  (add-to-list 'org-agenda-files
                                               dropbox-todo))))))
#+end_src
