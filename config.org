  ...
  (use-package tab-bar
    ...
    (defun --tab-bar-tab-name-fn ()
      ...
      (let ((buffer-name (-> (minibuffer-selected-window)
                             (window-buffer)
                             (buffer-name)))
            (project-name (project-root (project-current))))
        (if project-name
            (format "%s(%s)" buffer-name project-name)
  ...
  (use-package project
    ...
    (defun --compile-command-completing-read ()
      ...
      (let ((command (--completing-read "Compile command: "
            ...
            ;; We do this because newer versions of `project-current'
            ;; returns the form (vc Git "path"), when older versions
            ;; return instead (vc "path"). This pattern match handles
            ;; the difference
            (default-directory (-let (((_ a b) (project-current)))
                                 (cond
                                  ((stringp a) a)
                                  ((stringp b) b)
                                  (t default-directory)))))
    ...
    (defun --project-find-readme ()
      ...
      (let* ((case-fold-search t) ; ignore cases
             ...
             (project-dir (-let (((_ a b) (project-current)))
                            (cond
                             ((stringp a) a)
                             ((stringp b) b))))
...
#+begin_src emacs-lisp :results output silent
  (use-package vterm
    :straight t
    :if (-any (lambda (s) (eq system-type s))
              '(darwin gnu/linux))
    :custom
