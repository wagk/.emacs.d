...
(use-package tab-bar
  ...
  (tab-bar-tab ((t (:inherit default :italic t :foreground unspecified :background unspecified))))
  ...
  (with-eval-after-load 'corfu
    ...
    (cl-defun --lsp-mode-completion ()
      (setf (->> completion-category-defaults
                 ...
                 (alist-get 'styles))
            '(flex)))
    (add-hook 'lsp-completion-mode #'--lsp-mode-completion))
  :config
    ...
    (let* ((case-fold-search t) ; ignore cases
           ...
           (readme-list (->> (project-files (project-current))
                             ...
                             (mapcar #'(lambda (file)
                                         (f-relative file
                                          project-dir))))))
      (find-file (if (eq 1 (length readme-list))
                     (car readme-list)
...
(use-package deft
  ...
  :disabled t ;; try out consult-notes
  ;; :commands (deft
  ;;            deft-filter
...
(use-package chatgpt-shell
  ...
  (chatgpt-shell-display-function #'(lambda (buf)
                                      (pop-to-buffer buf
  ...
  (customize-set-variable
   ...
   (s-join
    ...
    '("Always show code snippets in markdown blocks with language labels."
      "Write summaries out first."
      "When implementing something in code, write tests for the implementation."
      "When asked to explain something, use more technical language."
      "Always provide the degree of confidence you have in an answer being correct.")))
