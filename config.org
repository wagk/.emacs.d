...
#+begin_src emacs-lisp :results output silent
  (use-package org-pomodoro
    ...
    (org-pomdoro-length
     20
     "We'll up it back to 25 (and maybe more) later.")
    ;; guild wars 2 has some excellent notification sounds
    (org-pomodoro-finished-sound
     (at-user-init-dir "notification1.wav"))
    (org-pomodoro-short-break-sound
     (at-user-init-dir "notification2.wav"))
    (org-pomodoro-long-break-sound
     (at-user-init-dir "notification3.wav"))
    :init
    (evil-ex-define-cmd "doing" 'org-clock-goto)
    (evil-ex-define-cmd "do" 'org-pomodoro))
    ;; :config
...
#+begin_src emacs-lisp :results output silent
  (with-eval-after-load 'hydra
    (with-eval-after-load 'org-brain
      (defhydra hydra-org-brain (org-brain-visualize-mode-map "")
        "org-brain bindings"
        ("m" org-brain-visualize-mind-map "toggle mind map vis" :exit t)
        ("b" org-brain-visualize-back "go to last node" :exit t)
        ("h" org-brain-add-child-headline "add child headline" :exit t)
        ("*" org-brain-add-child-headline "add child headline" :exit t)
        ("c" org-brain-add-child "add child" :exit t)
        ("C" org-brain-remove-child "remove child" :exit t)
        ("e" org-brain-annotate-edge "annotate edge" :exit t)
        ("p" org-brain-add-parent "add parent" :exit t)
        ("P" org-brain-remove-parent "remove parent" :exit t)
        ("f" org-brain-add-friendship "add friendship" :exit t)
        ("F" org-brain-remove-friendship "remove friendship" :exit t)
        ("n" org-brain-pin "pin entry" :exit t)
        ("s" org-brain-select-dwim "select entry for batch processing" :exit t)
        ("S" org-brain-select-map "prefix for batch process selected entries" :exit t)
        ("t" org-brain-set-title "edit title" :exit t)
        ("T" org-brain-set-tags "edit tags" :exit t)
        ("d" org-brain-delete-entry "delete node" :exit t)
        ("l" org-brain-visualize-add-resource "add new resource to entry" :exit t)
        ("r" org-brain-open-resource "open resource" :exit t)
        ("C-y" org-brain-visualize-paste-resource "add resource from keyboard" :exit t)
        ("a" org-brain-visualize-attach "org-attach on headline" :exit t)
        ("A" org-brain-archive "archive entry" :exit t)
        ("o" org-brain-goto-current "open current node" :exit t)
        ("O" org-brain-goto "open a node" :exit t)
        ("v" org-brain-visualize "see another entry" :exit t)
        ("w" org-brain-visualize-random "see random entry" :exit t)
        ("W" org-brain-visualize-wander "see random entries on timer" :exit t))))
#+end_src
...
#+begin_src emacs-lisp :results output silent
  (use-package yasnippet
    ...
    :custom
    (yas-snippet-dirs (list (at-user-init-dir "/snippets")))
...
** Prism mode
Much like rainbow blocks mode, but better somehow
#+begin_src emacs-lisp :results output silent
  (use-package prism
    :straight (:host github :repo "alphapapa/prism.el")
    :disabled t ;; needs "anaphora" or something like that
    :commands (prism-mode
               prism-set-faces)
    :preface
...
#+begin_src emacs-lisp :results output silent
  (use-package elisp-mode
    :hook ((emacs-lisp-mode . rainbow-delimiters-mode)
           (emacs-lisp-mode . rainbow-blocks-mode-enable)
...
#+begin_src emacs-lisp :results output silent
  (use-package clojure-mode
    ...
    :hook ((clojure-mode . rainbow-delimiters-mode)
           (clojure-mode . rainbow-blocks-mode-enable)
...
#+begin_src emacs-lisp :results output silent
  (use-package rg
    ...
    (evil-define-command config-rg-explicit (arg)
      (interactive "<a>")
                                          ;TODO: Figure out how to programmatically generate a files list, which we need in addition to a query
      (cond
    ...
    (evil-ex-define-cmd "rg" 'config-rg-explicit))
#+end_src
* C++
** Indentation of 4
#+begin_src emacs-lisp :results output silent
