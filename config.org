...
#+begin_src emacs-lisp :results output silent
  (use-package org-clock
    ...
    :init
    (evil-define-command my-org-clocking ()
      (interactive)
      (cond
       ((org-clocking-p) (org-clock-out))
       ((org-at-heading-p) (org-clock-in))
       ((string-equal (buffer-name) "*Org Agenda*") (org-agenda-clock-in))
       (t (org-clock-in '(4)))))
    (evil-ex-define-cmd "clock" 'my-org-clocking)
    (evil-ex-define-cmd "clocks" #'(lambda () (interactive)
                                     (org-clock-goto t))))
