...
#+begin_src emacs-lisp :results output silent
  (use-package org-roam
    ...
    (defun --with-selected-org-roam-directory (org-roam-func)
      ...
      (let ((repos (or --org-roam-repository-list `(,org-roam-directory))))
        (ivy-read "repo: " repos
                  ...
                  :preselect t
                  :action (lambda (dir)
                            ;; it's insufficient to temporarily update
                            ;; org-roam-directory, as we don't want
                            ;; every `org-roam' function to ask for the
                            ;; directory. Updating the default state
                            ;; should prevent surprises in this case.
                            (customize-set-variable 'org-roam-directory dir)
                            (org-roam-db-sync)
    ...
    (defun --with-selected-org-roam-directory-dailies (dailies-func)
      "Same as `--with-selected-org-roam-directory' except that this
  function also ensures that `org-roam-dailies-directory' is created."
      (--with-selected-org-roam-directory #'(lambda ()
                                              (let ((dailies-dir (f-join org-roam-directory
                                                                         org-roam-dailies-directory)))
                                                (unless (f-exists-p dailies-dir)
                                                  (make-directory dailies-dir t)))
                                              (funcall dailies-func))))

    (defun --org-roam-dailies-yesterday ()
      (interactive)
      (--with-selected-org-roam-directory-dailies #'(lambda () (org-roam-dailies-goto-yesterday 1))))

    (defun --org-roam-dailies-today ()
      (interactive)
      (--with-selected-org-roam-directory-dailies #'org-roam-dailies-goto-today))

    (defun --org-roam-dailies-tomorrow ()
      (interactive)
      (--with-selected-org-roam-directory-dailies #'(lambda () (org-roam-dailies-goto-tomorrow 1))))
...
#+begin_src emacs-lisp :results output silent
  (use-package magit-todos
    ...
    (magit-todos-branch-list-toggle t)
    :general
    (:keymaps
...
#+begin_src emacs-lisp :results output silent
  (use-package smudge
    ...
    ;; :straight t
    :if (and (bound-and-true-p my-spotify-oauth2-client-secret)
             (bound-and-true-p my-spotify-oauth2-client-id))
    :custom
  ...
  "
    ...
    (evil-ex-define-cmd "spot[ify]" #'hydra-spotify/body)
