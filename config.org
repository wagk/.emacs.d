...
#+begin_src emacs-lisp :results output silent
  (use-package aes
    ...
    :init
    ;; TODO:
    (defun --encrypt-org-subtree ()
      "Encrypt the contents of closest parent headline; or buffer
      if we're at the root of the org document. The headline would be
      the password, or the filename if it's the root"
      (-let (((beg end _) (evil-org-inner-subtree))
             (key (f-filename (f-this-file)))
             (heading (->> (org-get-heading nil nil nil nil)
                           (substring-no-properties)
                           (downcase)
                           (replace-regexp-in-string " " "-"))))
          (message "converted heading: %s" heading)))
          ;; (narrow-to-region beg end)))
          ;; TODO: take this and process the buffer
...
#+begin_src emacs-lisp :results output silent
  (use-package engine-mode
    ...
    :commands defengine
    :init
    (evil-define-command --ddg-search (args)
      (interactive "<a>")
      (require 'engine-mode)
