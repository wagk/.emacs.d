...
#+begin_src emacs-lisp :results output silent
  (when (boundp 'config-local-syncthing-folder)
      ...
      (defconst my-notes-org-file (concat config-local-syncthing-folder "/notes.org")))
#+end_src
...
#+begin_src emacs-lisp :results output silent
  ;; (customize-set-variable 'initial-buffer-choice (if (boundp 'my-todo-org-file)
  ;;                                                    my-todo-org-file
  ;;                                                  user-config-file))
#+end_src
...
#+begin_src emacs-lisp :results output silent
  (use-package org-agenda
    ...
    (when (boundp 'config-local-dropbox-folder)
      (let ((todo (f-join config-local-dropbox-folder "todo.org")))
        (when (f-exists-p todo)
          (customize-set-variable 'org-agenda-files
                                  (add-to-list 'org-agenda-files todo)))))
    (when (boundp 'config-local-syncthing-folder)
      (let ((todo (f-join config-local-syncthing-folder "todo.org")))
        (when (f-exists-p todo)
          (customize-set-variable 'org-agenda-files
                                  (add-to-list 'org-agenda-files todo))))))
#+end_src
...
#+begin_src emacs-lisp :results output silent
  (use-package eyebrowse
    ...
    (evil-define-command my-new-evil-tab (file-or-name)
      "Note that :h :tabe in vim indicates that it's file only, not
  buffer."
      (interactive "<a>")
      (require 'eyebrowse)
      (let ((eyebrowse-new-workspace (if (file-exists-p file-or-name)
                                         (lambda () (find-file file-or-name))
                                       eyebrowse-new-workspace))
            (tabname (if (file-exists-p file-or-name)
                         (file-name-base file-or-name)
                       file-or-name)))
        (eyebrowse-create-window-config)
        (my-rename-eyebrowse-tab tabname)))
    ...
    (evil-define-command my-rename-eyebrowse-tab (name)
      (interactive "<a>")
      (require 'eyebrowse)
      (eyebrowse-rename-window-config (frame-parameter nil 'eyebrowse-current-slot)
                                      name))
...
#+begin_src emacs-lisp :results output silent
  (use-package writeroom-mode
    ...
    (:states 'normal
     "g z" 'writeroom-mode)
    :hook
    ((writeroom-mode . (lambda () (require 'focus)
                         (if writeroom-mode
                             (progn (focus-init)
...
#+begin_src emacs-lisp :results output silent
  (when (boundp 'my-todo-org-file)
    ...
    (evil-ex-define-cmd "Ttodo" '(lambda () (interactive)
                                   ...
                                   (my-rename-eyebrowse-tab "todo"))))
#+end_src

#+begin_src emacs-lisp :results output silent
  (when (boundp 'my-notes-org-file)
    (my-evil-define-split-vsplit-cmd "notes" #'(lambda () (find-file my-notes-org-file)))
