...
#+begin_src emacs-lisp :results output silent
  (use-package org-roam
    ...
    :preface
    (customize-set-value 'org-roam-database-connector 'sqlite3)
    :custom
...
#+begin_src emacs-lisp :results output silent
  (defun --hacked-calculate-lisp-indent (&optional parse-start)
  ...
  (advice-add #'calculate-lisp-indent :override #'--hacked-calculate-lisp-indent)
#+end_src
...
#+begin_src emacs-lisp :results output silent
  (my-evil-define-split-vsplit-cmd "scratch" 'scratch-buffer)
  (evil-ex-define-cmd "TScratch" #'(lambda (interactive)
...
#+begin_src emacs-lisp :results output silent
  (use-package scratch
    ...
    (evil-define-command --scratch (&optional bang)
      ...
      (interactive "<!>")
      (require 'scratch)
      (let ((current-prefix-arg (when bang 4)))
  ...
  (use-package project
    ...
    (defun --find-project-todo-file ()
      ...
      (if-let ((proj-current (project-current))
               (todo (f-join (project-root proj-current) ".todo.org")))
          (find-file-other-window todo)
        (error "No todo.org found in %s" default-directory)))
...
#+begin_src emacs-lisp :results output silent
  (use-package vterm
    :straight t
    :if (and
         (not (bound-and-true-p --is-apple-m1-machine)) ; https://github.com/akermu/emacs-libvterm/issues/593
         (-any (lambda (s) (eq system-type s))
               '(darwin gnu/linux)))
    :custom
...
#+begin_src emacs-lisp :results output silent
  (use-package dirvish
    :straight t
    :disabled t ;; There's some issues with evil-mode integration here
    :custom-face
    (dirvish-hl-line ((((background dark)) (:background ,sol-base02))
                      (((background light)) (:background ,sol-base2))))
    :config
    (dirvish-override-dired-mode)
    (add-to-list 'evil-motion-state-modes 'dirvish-mode))
#+end_src
