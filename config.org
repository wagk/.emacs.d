* TODO Issues
=evil-insert-state-keymap= has a binding ~C-k~ that is currently
shadowing the helm mode maps. A proposed solution is to migrate to
~M-j~ and ~M-k~. These maps are also shadowing ~C-h~ and ~C-l~, which
is getting annoying.

The help+ packages are also not being autoloaded, which is also
getting annoying.
* Packaging
